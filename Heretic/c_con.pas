...
begin
  ...
  begin
    ...
    try
      ...
      str.Add('// These console commands will be executed everytime you start DelphiHeretic.');
      ...
      begin
        ...
        begin
          ...
          case c of
            ...
            begin
              c := Ord(toupper(Chr(c)));
              if ((c >= Ord(HU_FONTSTART)) and (c <= Ord(HU_CFONTEND))) or
                 (Chr(c) in [' ', '.', '!', '-', '+', '=', '*', '/', '\']) then
              begin
                if shiftdown then
                  c := Ord(shiftxform[c]);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (c >= HU_FONTSTART) and (c <= HU_CFONTEND) then
          begin
            patch := hu_font3[Ord(c) - Ord(HU_FONTSTART)];
    ...
    begin
      ...
      if (c >= HU_FONTSTART) and (c <= HU_CFONTEND) and (x < xmax) then
      begin
        patch := hu_font3[Ord(c) - Ord(HU_FONTSTART)];
        if c in FIXED_PITCH_CHARS then
        begin
