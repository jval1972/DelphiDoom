...
begin
  ...
  begin
    doomwaddir := getenv('HERETICWADDIR');
    if doomwaddir <> '' then
  ...
  begin
    doomwadpath := getenv('HERETICWADPATH');
    if doomwadpath <> '' then
    begin
      tmp := '';
...
begin
  // Shareware
  heretic1wad := D_FileInDoomPath('heretic1.wad');
...
// Try to guess minimum zone memory to allocate
  ...
  begin
    ...
    if lump >= 0 then
    begin
      if strupper(GetLumpCRC32(lump)) <> '1468BDEC' then
