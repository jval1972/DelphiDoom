...
// Find string width from hu_font chars
//
function M_StringWidth4(const str: string): integer;
var
  i: integer;
  c: integer;
begin
...
//      Find string height from hu_font chars
//
function M_StringHeight4(const str: string): integer;
var
  i: integer;
  height: integer;
begin
...
function M_WriteText4(x, y: integer; const str: string): menupos_t;
var
  w: integer;
  ch: integer;
  c: integer;
...
begin
  ...
  begin
    ...
    if (cx + w) > 320 then
      break;
    V_DrawPatch(cx, cy, SCN_TMP, hu_font4[c], false);
    cx := cx + w;
  end;
...
function M_WriteWhiteText4(x, y: integer; const str: string): menupos_t;
var
  w: integer;
  ch: integer;
  c: integer;
...
var
  KeyBindingsMenu1: array[0..Ord(kb_flyup) - 1] of menuitem_t;
  KeyBindingsDef1: menu_t;
  KeyBindingsMenu2: array[0..Ord(kb_weapon0) - Ord(kb_flyup) - 1] of menuitem_t;
...
const
  KeyBindingsInfo: array [0..Ord(kb_end) - 1] of bindinginfo_t = (
    ...
    (text: 'Staff'; pkey: @key_weapon0),
    (text: 'Elven Wand'; pkey: @key_weapon1),
    (text: 'Ethereal Crossbow'; pkey: @key_weapon2),
    (text: 'Dragon Claw'; pkey: @key_weapon3),
    (text: 'Hellstaff'; pkey: @key_weapon4),
    (text: 'Phoenix Rod'; pkey: @key_weapon5),
    (text: 'Firemace'; pkey: @key_weapon6),
    (text: 'Gauntlets of the Necromancer'; pkey: @key_weapon7)
  );
...
begin
  ...
  begin
    ppos := M_WriteText4(m.x, m.y + m.itemheight * i, KeyBindingsInfo[start + i].text);
    drawkey := true;
    if bindkeyEnter then
      if i = bindkeySlot - start then
        if (gametic div 18) mod 2 <> 0 then
          drawkey := false;
    if drawkey then
      M_WriteWhiteText4(ppos.x + 6, ppos.y, M_KeyToString(KeyBindingsInfo[start + i].pkey^));
  end;
end;

procedure M_DrawBindings1;
begin
  M_DrawBindings(KeyBindingsDef1, 0, Ord(kb_flyup));
end;

procedure M_DrawBindings2;
begin
  M_DrawBindings(KeyBindingsDef2, Ord(kb_flyup), Ord(kb_weapon0));
end;
...
begin
  ...
  bindkeySlot := Ord(kb_flyup) + choice;

  saveOldkey := KeyBindingsInfo[Ord(kb_flyup) + choice].pkey^;
end;
...
begin
  ...
  begin
    ...
    M_StartSound(nil, Ord(sfx_dorcls));
  end;
end;
...
begin
  if not usergame then
  begin
    M_StartSound(nil, Ord(sfx_plroof));
    exit;
  end;
...
begin
  ...
  ppos := M_WriteText4(ControlsDef.x, ControlsDef.y + ControlsDef.itemheight * Ord(ctrl_keyboardmode), 'Keyboard preset: ');
  M_WriteWhiteText4(ppos.x, ppos.y, mkeyboardmodes[M_GetKeyboardMode]);
end;
...
begin
  ...
  ppos := M_WriteText4(SystemDef.x, SystemDef.y + SystemDef.itemheight * Ord(sys_screenshottype), 'Screenshot format: ');
  M_WriteWhiteText4(ppos.x, ppos.y, screenshotformat);
end;
...
begin
  ...
  ppos := M_WriteText4(OptionsDisplayDetailDef.x, OptionsDisplayDetailDef.y + OptionsDisplayDetailDef.itemheight * Ord(od_detaillevel), 'Detail level: ');
  {$IFDEF OPENGL}
...
begin
  ...
  {$ENDIF}
  ppos := M_WriteText4(OptionsDisplayVideoModeDef.x, OptionsDisplayVideoModeDef.y + OptionsDisplayVideoModeDef.itemheight * Ord(odm_fullscreen), 'FullScreen: ');
  M_WriteWhiteText4(ppos.x, ppos.y, {$IFDEF OPENGL}yesnoStrings[fullscreen]{$ELSE}strfullscreenmodes[fullscreen]{$ENDIF});

  if mdisplaymode_idx < 0 then
    mdisplaymode_idx := 0
  else if mdisplaymode_idx >= numdisplaymodes then
    mdisplaymode_idx := numdisplaymodes - 1;
  ppos := M_WriteText4(OptionsDisplayVideoModeDef.x, OptionsDisplayVideoModeDef.y + OptionsDisplayVideoModeDef.itemheight * Ord(odm_screensize), 'Screen Size: ');
  ...
  M_WriteText4(OptionsDisplayVideoModeDef.x, OptionsDisplayVideoModeDef.y + OptionsDisplayVideoModeDef.itemheight * Ord(odm_setvideomode), stmp);
end;
...
begin
  ...
  ppos := M_WriteText4(OptionsDisplayAppearanceDef.x, OptionsDisplayAppearanceDef.y + OptionsDisplayAppearanceDef.itemheight * Ord(od_shademenubackground), 'Menu background: ');
  M_WriteWhiteText4(ppos.x, ppos.y, menubackrounds[shademenubackground mod 3]);
end;
...
begin
  ...
  ppos := M_WriteText4(OptionsDisplayAspectRatioDef.x, OptionsDisplayAspectRatioDef.y + OptionsDisplayAspectRatioDef.itemheight * Ord(oda_forceaspectratio), 'Force Aspect Ratio: ');
  M_WriteWhiteText4(ppos.x, ppos.y, straspectratios[_nearest_aspect_index]);
end;
...
begin
  ...
  chasecamera_viewxy := ibetween(chasecamera_viewxy, CHASECAMERA_XY_MIN, CHASECAMERA_XY_MAX);
  ppos := M_WriteText4(OptionsDisplayCameraDef.x, OptionsDisplayCameraDef.y + OptionsDisplayCameraDef.itemheight * Ord(odc_chasecameraxy), 'Chase Camera XY position: ');
  M_WriteWhiteText4(ppos.x, ppos.y, itoa(chasecamera_viewxy));

  chasecamera_viewz := ibetween(chasecamera_viewz, CHASECAMERA_Z_MIN, CHASECAMERA_Z_MAX);
  ppos := M_WriteText4(OptionsDisplayCameraDef.x, OptionsDisplayCameraDef.y + OptionsDisplayCameraDef.itemheight * Ord(odc_chasecameraz), 'Chase Camera Z position: ');
  M_WriteWhiteText4(ppos.x, ppos.y, itoa(chasecamera_viewz));
...
begin
  ...
  ppos := M_WriteText4(OptionsDisplay32bitDef.x, OptionsDisplay32bitDef.y + OptionsDisplay32bitDef.itemheight * Ord(od_flatfiltering),
    'Flat filtering: ');
  M_WriteWhiteText4(ppos.x, ppos.y, flatfilteringstrings[extremeflatfiltering]);
end;
...
begin
  ...
  vx_maxoptimizerpasscount := GetIntegerInRange(vx_maxoptimizerpasscount, 0, MAX_VX_OPTIMIZE);
  ppos := M_WriteText4(OptionsDisplayOpenGLVoxelsDef.x, OptionsDisplayOpenGLVoxelsDef.y + OptionsDisplayOpenGLVoxelsDef.itemheight * Ord(od_glv_optimize), 'Voxel mesh optimization: ');
  M_WriteWhiteText4(ppos.x, ppos.y, str_voxeloptimizemethod[vx_maxoptimizerpasscount]);
end;
...
begin
  ...
  ppos := M_WriteText4(OptionsDisplayOpenGLFilterDef.x, OptionsDisplayOpenGLFilterDef.y + OptionsDisplayOpenGLFilterDef.itemheight * Ord(od_glf_texture_filter), 'Filter: ');
  M_WriteWhiteText4(ppos.x, ppos.y, gl_tex_filter_string);
end;
{$ENDIF}
...
begin
  ...
  begin
    case ch of
      ...
      begin
        ...
        if ch <> 32 then
        if (ch - Ord(HU_FONTSTART) < 0) or (ch - Ord(HU_FONTSTART) >= HU_FONTSIZE4) then
        else
        begin
  ...
  begin
    ...
    menuactive := false;
    M_StartSound(nil, Ord(sfx_switch));
    exit;
  end;
    ...
    case ch of
      KEY_MINUS:    // Screen size down
        begin
          ...
          M_SizeDisplay(0);
          M_StartSound(nil, Ord(sfx_keyup));
          result := true;
          exit;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ppos := M_WriteText4(x, y, str + ': ');
          M_WriteWhiteText4(ppos.x, ppos.y, yesnoStrings[currentMenu.menuitems[i].pBoolVal^]);
        end
        else
...
////////////////////////////////////////////////////////////////////////////////
//KeyBindingsDef
  KeyBindingsDef1.numitems := Ord(kb_flyup); // # of menu items
...
//KeyBindings2Menu
  ...
  for i := 0 to Ord(kb_weapon0) - Ord(kb_flyup) - 1 do
  begin
    pmi.status := 1;
    pmi.name := '!' + KeyBindingsInfo[Ord(kb_flyup) + i].text + ': ';
    pmi.cmd := '';
...
////////////////////////////////////////////////////////////////////////////////
//KeyBindings2Def
  KeyBindingsDef2.numitems := Ord(kb_weapon0) - Ord(kb_flyup); // # of menu items
