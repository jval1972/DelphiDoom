...
begin
  result := -1;
  if sfxinfo.name = '' then
    exit;

  if sfxinfo.name[1] = '-' then
  begin
    namebuf := Copy(sfxinfo.name, 2, Length(sfxinfo.name) - 1);
  ...
  // JVAL, search without the ds prefix
...
begin
  ...
  begin
    pDS := nil;
    I_Warning('I_InitSound(): DirectSoundCreate Failed, result = %d', [hres]);
    exit;
  end;
