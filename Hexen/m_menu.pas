...
begin
  ...
  begin
    ppos := M_WriteText3(KeyBindingsDef.x, KeyBindingsDef.y + KeyBindingsDef.itemheight * i, KeyBindingsInfo[start + i].text);
    drawkey := true;
    if bindkeyEnter then
      if i = bindkeySlot - start then
        if (gametic div 18) mod 2 <> 0 then
          drawkey := false;
    if drawkey then
      M_WriteWhiteText3(ppos.x + 6, ppos.y, M_KeyToString(KeyBindingsInfo[start + i].pkey^));
  end;
end;
...
begin
  ...
  chasecamera_viewxy := ibetween(chasecamera_viewxy, CHASECAMERA_XY_MIN, CHASECAMERA_XY_MAX);
  ppos := M_WriteText3(OptionsDisplayCameraDef.x, OptionsDisplayCameraDef.y + OptionsDisplayCameraDef.itemheight * Ord(odc_chasecameraxy), 'Chase Camera XY postion: ');
  M_WriteWhiteText3(ppos.x, ppos.y, itoa(chasecamera_viewxy));

  chasecamera_viewz := ibetween(chasecamera_viewz, CHASECAMERA_Z_MIN, CHASECAMERA_Z_MAX);
  ppos := M_WriteText3(OptionsDisplayCameraDef.x, OptionsDisplayCameraDef.y + OptionsDisplayCameraDef.itemheight * Ord(odc_chasecameraz), 'Chase Camera Z postion: ');
