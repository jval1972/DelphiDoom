...
/*==============================================================================
  ...
  $Id: load_it.c,v 1.27 1999/02/08 07:24:05 miod Exp $
...
BOOL IT_Init(void)
{
	...
	if(!(itpat=(ITNOTE*)_mm_malloc(256*64*sizeof(ITNOTE)))) return 0;
...
BOOL IT_Load(BOOL curious)
{
	...
	of.flags      |= UF_BGSLIDES;

	/* set the module type */
	/* 2.17 : don't know but found one module... */
	/* 2.16 : IT 2.14p3 with resonant filters */
	/* 2.15 : IT 2.14p3 (improved compression) */
	if((mh->cwt<=0x219)&&(mh->cwt>=0x215))
	...
	for(t=0;t<mh->ordnum;t++) {
		...
		if((of.positions[t]>mh->patnum)&&(of.positions[t]<254))
			of.positions[t]=255;
	}
		...
		for(t=0;t<mh->insnum;t++) {
			...
			if(mh->cwt<0x200) {
				...
				for(lp=0;lp<ITENVCNT;lp++) {
					...
					/* load IT 2xx vol & chanpan & pitch envs */
				ih.volflg    = _mm_read_UBYTE(modreader);
