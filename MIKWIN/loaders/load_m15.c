...
/*==============================================================================
  ...
  $Id: load_m15.c,v 1.17 1999/02/08 07:24:06 miod Exp $
...
#define REJECT 3
/* Flight Simulator .mod files */
static char flightsimulator[5]={0x0e,0x02,0x17,0x00,0x01};
static char *signatures[REJECT]={
	flightsimulator,
	"CAKEWALK",	/* cakewalk midi files */
	"SZDD"		/* Microsoft compressed files */
};
static int siglen[REJECT]={5,8,4};
...
static BOOL M15_Test(void)
{
	...
	for(t=0;t<15;t++) {
		...
		if(mh.samples[t].samplename[0]=='s') {
		  ...
		  if((mh.samples[t].samplename[0]<'0')||
	...
	if(instrument) {
		...
		if((instrument>15)||(!mh->samples[instrument-1].length))
			...
			/* ...otherwise, only adjust volume... */
				...
				/* ...unless a volume effect was specified, which forces a
