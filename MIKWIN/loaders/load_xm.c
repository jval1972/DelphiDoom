...
/*==============================================================================
  ...
  $Id: load_xm.c,v 1.20 1999/02/08 07:24:14 miod Exp $
...
static void XM_ReadNote(XMNOTE* n)
{
	...
	if(cmp&0x80) {
		if(cmp&1)  n->note = _mm_read_UBYTE(modreader);
		if(cmp&2)  n->ins  = _mm_read_UBYTE(modreader);
		if(cmp&4)  n->vol  = _mm_read_UBYTE(modreader);
		if(cmp&8)  n->eff  = _mm_read_UBYTE(modreader);
		if(cmp&16) n->dat  = _mm_read_UBYTE(modreader);
	} else {
		n->note = cmp;
...
BOOL XM_Load(BOOL curious)
{
	...
	for(t=20;tracker[t]<=' ';t--) tracker[t]=0;
	...
	of.numins    = mh->numins;
	of.flags |= UF_XMPERIODS | UF_INST | UF_BGSLIDES;
