...
/*==============================================================================
  ...
  $Id: load_imf.c,v 1.2 1999/02/08 07:24:16 miod Exp $
	...
	if((eff)&&(eff!=255))
		switch (eff) {
			...
			case 0x0e:	/* fine volume slide */
				if(inf>>4)
					UniPTEffect(0xe,0xa0|(inf>>4));
				else
					UniPTEffect(0xe,0xb0|inf);
				break;
			...
			case 0x14:	/* fine slide up */
				UniPTEffect(0xe,0x10|(inf>>4));
				break;
			case 0x15:	/* fine slide down */
				UniPTEffect(0xe,0x20|(inf>>4));
				break;
			...
			case 0x21:	/* extended effects */
				switch(inf>>4) {
					...
					case 0xe:	/* ignore envelope */
						fprintf(stderr,"\reffect 0x21[Ex] (ignore envelope)"
						               " not implemented (inf=%2X)\n",inf);
						break;
#endif
				}
...
static UBYTE* IMF_ConvertTrack(IMFNOTE* tr,UWORD rows)
{
	...
	for(t=0;t<rows;t++) {
		...
		if(note!=255) {
			if(note==0xa0) UniPTEffect(0xc,0);
			else UniNote(((note>>4)*OCTAVE)+(note&0xf));
		}
...
#define IMF_ProcessEnvelope(name) \
	...
	for(u=0;u<of.numsmp;u++,s++,q++) {
		...
		if(s->flags&0x4) q->flags|=SF_16BITS;
