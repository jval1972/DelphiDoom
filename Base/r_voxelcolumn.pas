...
procedure R_DrawVoxelColumnLow;
procedure R_DrawVoxelColumnMedium;
procedure R_DrawVoxelColumnHi;
{$IFNDEF STRIFE}
procedure R_DrawVoxelFuzzColumn;
procedure R_DrawVoxelFuzzColumn32;
procedure R_DrawVoxelFuzzColumnHi;
{$ENDIF}
procedure R_DrawVoxelTranslatedColumn;
procedure R_DrawVoxelTranslatedColumnHi;
{$IFDEF DOOM_OR_STRIFE}
procedure R_DrawVoxelColumnAverageMedium;
{$ENDIF}
procedure R_DrawVoxelColumnAverageHi;
{$IFDEF STRIFE}
procedure R_DrawVoxelFuzzColumn1;
procedure R_DrawVoxelFuzzColumn2;
procedure R_DrawVoxelFuzzColumnTL;
procedure R_DrawVoxelFuzzColumn1Hi;
procedure R_DrawVoxelFuzzColumn2Hi;
procedure R_DrawVoxelFuzzColumnHiTL;
{$ENDIF}
{$IFDEF DOOM_OR_STRIFE}
procedure R_DrawVoxelColumnAlphaMedium;
{$ENDIF}
procedure R_DrawVoxelColumnAlphaHi;

procedure R_DrawVoxelColumnAddMedium;
procedure R_DrawVoxelColumnAddHi;

procedure R_DrawVoxelColumnSubtractMedium;
procedure R_DrawVoxelColumnSubtractHi;

var
// JVAL: voxel column drawing
  dc_voxeliscale: fixed_t;
...
begin
  ...
  frac := (dc_yl - centery) * dc_voxeliscale;
  fracstep := 3 * dc_voxeliscale;
  swidth := SCREENWIDTH - num_batch_columns;

  for i := 0 to count - 1 do
  begin
    bdest := dc_colormap[dc_source[(LongWord(frac) shr FRACBITS)]];
  ...
  begin
    ...
    begin
      ...
      //  using a lighting/special effects LUT.
        bdest := dc_colormap[dc_local[(LongWord(frac) shr FRACBITS)]];
...
begin
  ...
  //  using a lighting/special effects LUT.
    ...
    b := dc_colormap[dc_source[(LongWord(frac) shr FRACBITS)]];
    u := b shl 8;
    cnt := num_batch_columns;
    while cnt > 0 do
    begin
...
begin
  ...
  while count >= 0 do
  begin
    c3 := dc_colormap32[dc_source[(LongWord(frac) shr FRACBITS)]];
    r1 := c3;
    g1 := c3 shr 8;
    b1 := c3 shr 16;
...
begin
  ...
  while frac <= fraclimit do
  begin
    c2 := dc_colormap32[dc_source[(LongWord(frac) shr FRACBITS)]];
    r2 := c2;
    g2 := c2 shr 8;
    b2 := c2 shr 16;
    fr2 := cfrac2 * r2;
...
begin
  ...
  //  using a lighting/special effects LUT.
    ...
    u := dc_colormap[dc_source[(LongWord(frac) shr FRACBITS)]];
    cnt := num_batch_columns;
    while cnt > 0 do
    begin
...
begin
  ...
  //  using a lighting/special effects LUT.
    ...
    u := dc_colormap[dc_translation[dc_source[(LongWord(frac) shr FRACBITS)]]];
    cnt := num_batch_columns;
    while cnt > 0 do
    begin
      dest^ := xlatab[dest^ * 256 + u];
...
begin
  ...
  while frac <= fraclimit do
  begin
    c2 := dc_colormap32[dc_translation[dc_source[(LongWord(frac) shr FRACBITS)]]];
    r2 := c2;
    g2 := c2 shr 8;
    b2 := c2 shr 16;
    fr2 := (3 * (FRACUNIT div 4)) * r2;
