...
var
    ...
    begin
      ...
      begin
        ...
        case VarP^.FType.BaseType of
          ...
          btChar,
          btU8:
            if not read(PPSVariantU8(VarP)^.data, 1) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          {$IFNDEF PS_NOINT64}
          bts64:
            if not read(PPSVariantS64(VarP)^.Data, SizeOf(TbtS64)) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          {$ENDIF}
          btSingle:
            if not read(PPSVariantSingle(VarP)^.Data, SizeOf(TbtSingle)) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          btDouble:
            if not read(PPSVariantDouble(varp)^.Data, SizeOf(TbtDouble)) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          btExtended:
            if not read(PPSVariantExtended(varp)^.Data, SizeOf(TbtExtended)) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          btCurrency:
            if not read(PPSVariantExtended(varp)^.Data, SizeOf(TbtCurrency)) then
            begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
          ...
          btString:
            begin
              ...
              if not read(PPSVariantAString(varp)^.Data[1], NameLen) then
              begin
                CMD_Err(erOutOfRange);
                DestroyHeapVariant(VarP);
                Result := False;
          ...
          btWideString:
            begin
              ...
              if not read(PPSVariantWString(varp).Data[1], NameLen * 2) then
              begin
                CMD_Err(erOutOfRange);
                DestroyHeapVariant(VarP);
                Result := False;
          ...
          btUnicodeString:
            begin
              ...
              if not read(PPSVariantUString(varp).Data[1], NameLen * 2) then
              begin
                CMD_Err(erOutOfRange);
                DestroyHeapVariant(VarP);
                Result := False;
  ...
  begin
    ...
    begin
      if not read(currf, SizeOf(currf)) then
      begin
  ...
  begin
    ...
    begin
      if not read(Rec, SizeOf(Rec)) then
      begin
        cmd_err(erUnexpectedEof);
        LoadProcs := False;
        Exit;
      ...
      begin
        ...
        begin // exported
          ...
          if not read(TPSInternalProcRec(Curr).FExportName[1], L3) then
          begin
            Curr.Free;
            cmd_err(erUnexpectedEof);
            LoadProcs := False;
          ...
          if not read(TPSInternalProcRec(Curr).FExportDecl[1], L3) then
          begin
            Curr.Free;
            cmd_err(erUnexpectedEof);
            LoadProcs := False;
